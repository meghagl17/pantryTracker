    ...
    if (deleteItem) {
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          console.log(
            'No items found in the shopping list or collection does not exist.',
          );
        }

        // Reference to the store's shopping list subcollection
        const doneListRef = collection(
          db,
          'users',
          userId,
          'stores',
        ...
        if (!doneQuerySnapshot.empty) {
          // Delete each item in the shopping list
          const deletePromises2 = doneQuerySnapshot.docs.map((doc) =>
            deleteDoc(doc.ref),
          );
      ...
      >
        <React.Fragment>
          <Dialog open={openDialog}>
            ...
            <DialogContent>
              Are you sure you want to delete all items from the shopping list
              in this store? This action cannot be undone.
            </DialogContent>
            <DialogActions>
              <CustomButton onClick={() => handleCloseDialog(false)}>
                Cancel
              </CustomButton>
            ...
            <>
              <div>
                ...
                >
                  ...
                  >
                    <CustomButton onClick={() => goToGeneral()}>
                      Add items
                    </CustomButton>
                  </Box>
                </CustomCard>
              ...
              {storesList.map((item) => (
                ...
                >
                  ...
                  >
                    <CustomButton onClick={() => goToStore(item.id, item.name)}>
                      Add items
                    </CustomButton>
                  </Box>
                </CustomCard>
