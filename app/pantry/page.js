...
export default function Home() {
  ...
  const [clickedRequest, setClikedRequest] = useState({
    addItem: false,
    searchItem: true,
    getRecipe: false,
  });
  ...
  const handleClose = async (itemDelete, addtoShoppingCart) => {
    ...
    if (addtoShoppingCart) {
      ...
      const generalListCollectionRef = collection(
        db,
        'users',
        userId,
        'generalList',
      ...
      >
          ...
          <Dialog open={open}>
            ...
            <DialogContent>
              <DialogContentText id="alert-dialog-description">
                Since this item is not in your pantry anymore, do you want to
                add it to your shopping list? It will be added to the GENERAL
                shopping list.
              </DialogContentText>
            </DialogContent>
            <DialogActions>
              <CustomButton onClick={() => handleClose(itemDelete, false)}>
                NO
              </CustomButton>
              <CustomButton
                onClick={() => handleClose(itemDelete, true)}
                autoFocus
              >
                YES
              </CustomButton>
        ...
        <React.Fragment>
          <Dialog open={openForUpdate}>
            ...
            <DialogActions>
              <CustomButton onClick={() => handleCloseForUpdate(false)}>
                Cancel
              </CustomButton>
              <CustomButton
                onClick={() => handleCloseForUpdate(true)}
                autoFocus
              >
                Update
              </CustomButton>
        ...
        >
          {clickedRequest.searchItem ? (
            ...
            <Button
              ...
              onClick={handleSearchItemClicked}
            >
              Search Item
            </Button>
          )}
        ...
        >
          {pantry.length === 0 && loading === false ? (
            ...
            >
              ...
              >
                🛒 Your pantry is currently empty. Start adding items to keep
                track of your stock!
              </Typography>
            </Box>
          ) : null}
            ...
            ? pantry.map((item) => (
                <CustomCard key={item.id}>
                  ...
                  >
                    <CustomButton
                      onClick={() =>
                        handleClickOpenForUpdate(item.id, item.name)
                      }
                    >
                      Update Item
                    </CustomButton>
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <RemoveIcon
                        onClick={() => removeFromInventory(item.id, item.name)}
