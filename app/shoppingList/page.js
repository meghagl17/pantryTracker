...
export default function Home() {
  ...
  const handleChange = async (event, itemId, itemName) => {
    ...
    try {
      const storesCollectionRef = collection(
        db,
        'users',
        userId,
        'stores',
      ...
      console.log(
        `Item ${itemId} ${itemName}was moved to the store successfully`,
      );
    } catch (error) {
  ...
  return (
    <ProtectedRoute>
      ...
      >
        ...
        >
          {shoppingList.length === 0 && loading === false ? (
            <Box
              sx={{
                display: 'flex',
                  ...
                  <Box display="flex" alignItems="center">
                    ...
                    <div>
                      <FormControl sx={{ ml: 5, mt: 1, minWidth: 120 }}>
                        <InputLabel id={`select-label-${item.id}`}>
                          Store
                        </InputLabel>
                        <Select
